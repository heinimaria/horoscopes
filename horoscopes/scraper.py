...
def get_cancer():
    ...
    daily_cancer = soup_cancer.find('div', {'id': 'content'}).get_text()
    return daily_cancer


def get_gemini():
    soup_gemini = BeautifulSoup(url_gemini.text, 'html.parser')
    daily_gemini = soup_gemini.find('div', {'id': 'content'}).get_text()
    return daily_gemini


def get_pisces():
    soup_pisces = BeautifulSoup(url_pisces.text, 'html.parser')
    daily_pisces = soup_pisces.find('div', {'id': 'content'}).get_text()
    return daily_pisces
